	...
	public class SoundsManager extends EventDispatcher
	{
		...
		public function initSounds():void {
			if (Capabilities.hasAudio && Capabilities.hasMP3) {
				...
				_sounds.good_prize = new sound_get_key();
				_sounds.bad_prize = new sound_slow_time();				
				_sounds.life = new sound_crystal();
				_sounds.prize_break = new sound_bleed();
				_sounds.click = new sound_click();
				_sounds.levelstart = new sound_buff();
				_sounds.win = new sound_win();
				_sounds.loose = new sound_loose();
			}
		}
		...
		private function upVolume(e:Event):void {
            ...
            if (_musicTransform.volume > 1.0) {
                ...
                App.root.removeEventListener(Event.ENTER_FRAME, upVolume, false);
            }
            ...
            if (_nextMusicId != null) {
                App.root.removeEventListener(Event.ENTER_FRAME, upVolume, false);
                App.root.addEventListener(Event.ENTER_FRAME, downVolume, false, 0, true);
            }
        }
		...
		private function downVolume(e:Event):void {
			...
			if (_musicTransform.volume < 0.0) {
				...
				App.root.removeEventListener(Event.ENTER_FRAME, downVolume, false);
				if (_nextMusicId != null) {
		...
		private function initMusic(id:String):void {
			...
			App.root.addEventListener(Event.ENTER_FRAME, upVolume, false, 0, true);
		}
